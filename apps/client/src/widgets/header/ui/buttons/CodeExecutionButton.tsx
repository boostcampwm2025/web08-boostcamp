...
export function CodeExecutionButton() {
  ...
  // Check if current file supports code execution
  const fileName = activeFileId ? getFileName(activeFileId) : null;
  const extension = fileName ? extname(fileName) : null;
  const language = extension ? getPistonLanguage(extension) : null;

  // Disable button if no language support or already executing
  const isDisabled = !activeFileId || !language || isExecuting;
